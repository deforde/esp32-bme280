...
bool i2c_init()
{
    ...
    CHECK_ESP_OK(i2c_driver_install(I2C_PORT_NUM, I2C_MODE_MASTER, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0));
...
bool i2c_write_reg(uint8_t dev_addr, uint8_t reg_addr, uint8_t reg_val)
{
    ...
    CHECK_ESP_OK(i2c_master_write_byte(cmd, (dev_addr << 1) | I2C_MASTER_WRITE, I2C_ACK_CHECK_EN));
    CHECK_ESP_OK(i2c_master_write(cmd, data, sizeof(data), I2C_ACK_CHECK_EN));
    CHECK_ESP_OK(i2c_master_stop(cmd));
    esp_err_t ret = i2c_master_cmd_begin(I2C_PORT_NUM, cmd, I2C_WRITE_TIMEOUT_MS / portTICK_RATE_MS);
...
bool i2c_read_reg(uint8_t dev_addr, uint8_t reg_addr, uint8_t* reg_val)
{
    ...
    CHECK_ESP_OK(i2c_master_stop(cmd));
    ret = i2c_master_cmd_begin(I2C_PORT_NUM, cmd, I2C_WRITE_TIMEOUT_MS / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    CHECK_ESP_OK(ret);

    cmd = i2c_cmd_link_create();
    CHECK_ESP_OK(i2c_master_start(cmd));
    CHECK_ESP_OK(i2c_master_write_byte(cmd, (dev_addr << 1) | I2C_MASTER_READ, I2C_ACK_CHECK_EN));
